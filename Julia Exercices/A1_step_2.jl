...
@variable(FN,p_w_grid[t=1:T,w=1:W]>=0) #wind farm production for grid
@variable(FN,p_w_H2[t=1:T,w=1:W]>=0) #wind farm production for H2
...
@variable(FN,H2[t=1:T,h=1:2]>=0) #H2 prod

@objective(FN, Max, sum(U_d[t,d]*p_d[t,d] for t=1:T,d=1:D)  #Revenue from demand
            + H2_price*sum(p_w_H2[t,w]*H2_prod for t=1:T, w=1:2)
            - sum(run[t,g]*C_g[g]*p_g[t,g] for t=1:T,g=1:G) # Production cost conventional generator
            - sum(0*(p_w_grid[t,w] + p_w_H2[t,w]) for t=1:T,w=1:W)) #Maximize the social whalefare, /# Production cost Wind farm
...
@constraint(FN,[t=1:T, w=1:2], p_w_H2[t,w] <= WF_cap[w]/2) #power from windfarm not above capacity of electrolyzer
@constraint(FN,[t=1:T, w=3:W], p_w_H2[t,w] == 0) #set all other WF to 0 prod of H2
@constraint(FN, Balance[t=1:T], sum(p_d[t,d] for d=1:D) - sum(p_w_grid[t,w] for w=1:W) - sum(run[t,g]*p_g[t,g] for g=1:G)==0) #Power balance constraint
@constraint(FN, [t=1:T, g=1:G], st[t,g] >= run[t,g] - (t>1 ? run[t-1,g] : run[T,g]))

@constraint(FN, [w=1:2], sum(p_w_H2[t,w]*H2_prod for t=1:T) >= H2_cap) # minimum of H2 prod WF 4
