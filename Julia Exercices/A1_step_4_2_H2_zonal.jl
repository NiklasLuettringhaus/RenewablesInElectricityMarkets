...
@variable(FN, wind_cur[w=1:W]>=0) #wind farm curtailment if electrolyzer
@variable(FN, outage[g=1:G]>=0)    #
...
@objective(FN, Max, sum(U_d[d]*p_d[d] for d=1:D)              #Revenue from demand
            #- sum(cost_load_cur*load_cur[d] for d=1:D)         #curtailment cost load
...
# DA Power Balance
@constraint(FN, Balance, sum(p_d[d] for d=1:D) 
                                #+ sum(f[a,b] for b=1:A if ATC[a,b]>0) 
                                - sum(p_w_grid_DA[w] for w=1:W) 
                                - sum(p_g[g] for g=1:G)
                                ==0)

#Ramping up and down constraints
#@constraint(FN,[g=1:G], p_g[g] <= (t-1<1 ? Cap_g_init[g] : p_g[t-1,g]) + Ramp_g_u[g]) #ramp up constraint
#@constraint(FN,[g=1:G], p_g[g] >= (t-1<1 ? Cap_g_init[g] : p_g[t-1,g]) - Ramp_g_d[g]) #ramp down constraint

#=
#Power Flow constraints
@constraint(FN,[a=1:A,b=1:A],f[a,b]<=ATC[a,b]) # Max ATC connecting zones a to b
@constraint(FN,[a=1:A,b=1:A],f[a,b]>=-ATC[b,a]) # Min ATC connecting zones a to b
@constraint(FN,[a=1:A,b=1:A],f[a,b]==-f[b,a]) #Symmetrical flow
=#
...
#Balancing action
#@constraint(FN, [t=1:T], outage[7] == p_g[7])                   #outage of generator 7
#@constraint(FN, [t=1: g=1,2,3,4,5,6,8,9,10,11,12], outage[g] == 0)     #
@constraint(FN, [w=1:W; outage_sum >= 0], outage_sum <= sum(wind_cur[w] for w=1:W) + sum(down_bal[g] for g=1:G))    #down balancing action of electrolyzer needs to match WF error
...
if termination_status(FN) == MOI.OPTIMAL
    ...
    println("Cost of hydrogen production: ", round(value(sum(DA_price[t]*p_w_H2_DA[w] for w=1:2)), digits = 2))
    println("Cost of up balancing: ", round(value(sum(up_bal_w_H2[w]*0.85*DA_price[t] for w=1:2)), digits = 2))
    println("Cost of down balancing: ", round(value(sum(down_bal_w_H2[w]*1.1*DA_price[t] for w=1:2)), digits = 2))
