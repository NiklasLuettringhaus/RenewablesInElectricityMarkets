...
@objective(FN, Max, sum(U_d[t,d]*p_d[t,d] for t=1:T,d=1:D)  #Revenue from demand
            ...
            - sum(DA_price[t]*p_w_H2[t,w] for t=1:T,w=1:2)) #Maximize the social whalefare, /# Production cost Wind farm
...
@constraint(FN,[t=1:T,g=1:G], p_g[t,g] <= (t=1 ? Cap_g_init[g] : p_g[t-1,g]) + Ramp_g_u[g]) #ramp up constraint
@constraint(FN,[t=1:T,g=1:G], p_g[t,g] >= (t=1 ? Cap_g_init[g] : p_g[t-1,g]) + Ramp_g_d[g]) #ramp down constraint

#Electrolyzer constraints
@constraint(FN,[t=1:T,w=1:2], p_w_H2[t,w] <= WF_cap[w]/2)
@constraint(FN,[w=1:2], sum(p_w_H2[t,w]*H2_prod for t=1:T) >= H2_cap)
...
if termination_status(FN) == MOI.OPTIMAL
    ...
    for w=1:W
        println("WF $w: ", round(Int,value(sum(p_w[t,w]*DA_price[t] for t=1:T))))
    end
    println("\n")
