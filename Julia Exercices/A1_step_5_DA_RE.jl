...
@variable(FN,up_g[t=1:T,g=1:G]>=0) #power scheduled of generetor g
@variable(FN,down_g[t=1:T,g=1:G]>=0) #power scheduled of generetor g
@variable(FN,up_e[t=1:T,w=1:2]>=0) #power scheduled of generetor g
@variable(FN,down_e[t=1:T,w=1:2]>=0) #power scheduled of generetor g
...
@objective(FN, Max, sum(U_d[t,d]*p_d[t,d] for t=1:T,d=1:D)              #Revenue from demand
            ...
            - sum(up_g[t,g] * c_res_g[t,g] for t=1:T, g=1:G)
...
@constraint(FN,[t=1:T,g=1:G], down_g[t,g] <= p_g[t,g]) #Generation limits constraint
@constraint(FN,[t=1:T,g=1:G], p_g[t,g] <= Cap_g[g] - up_g[t,g]) #Generation limits constraint
@constraint(FN,[t=1:T,w=1:W], p_w_grid_DA[t,w] + p_w_H2_DA[t,w] <= WF_prod[t,w]) #Weather-based limits constraint WF from DA market
@constraint(FN,[t=1:T,w=1:2], p_w_H2_DA[t,w] <= WF_cap[w]/2 - up_e[t,w]) # Electrolyzer can maximum do max capacity - reserve and must do min reserve down
@constraint(FN,[t=1:T,w=1:2], down_e[t,w] <= p_w_H2_DA[t,w]) # Electrolyzer can maximum do max capacity - reserve and must do min reserve down
