...
@variable(A2_11, delta[t=1:T, s in generated_values])        #total balancing in real time needed in balancing market

@objective(A2_11, Max, sum(prob[s] *    (scenarios[s][2][t] * p_DA[t] 
                                        + (1-scenarios[s][3][t]) * (delta_up[t,s] * scenarios[s][2][t] - delta_down[t,s] * coef_high * scenarios[s][2][t])  
                                        + scenarios[s][3][t] * (delta_up[t,s] * coef_low * scenarios[s][2][t] - delta_down[t,s] * scenarios[s][2][t])) for t=1:T, s in generated_values))
...
@constraint(A2_11,[t=1:T, s in generated_values], delta_up[t,s] <= p_nom)
@constraint(A2_11,[t=1:T, s in generated_values], delta_down[t,s] <= p_nom)
