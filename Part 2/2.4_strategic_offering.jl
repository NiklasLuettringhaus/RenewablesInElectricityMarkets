...
#Objective function
@objective(A2_22, Max,
- sum(p_s[s, sc]*C_s[s] for s=1:S, sc=1:SC) 
+ sum(alpha_bid[sc, k]*alpha_bid_fix[k]*d[k, sc] for k=1:K, sc=1:SC) 
- sum(alpha_offer_o[sc, o]*alpha_offer_o_fix[o]*p_o[o, sc] for o=1:O, sc=1:SC)
- sum(mu_up_k[k, sc]*D_max_k[k]*demand[sc,k] for k=1:K, sc=1:SC) 
- sum(mu_up_o[o,sc]*P_max_o[o] for o=2:O, sc=1:SC) - sum(mu_up_o[1, sc]*P_max_o[1]*wind_prod[sc,1] for sc=1:SC)
)
...
@constraint(A2_22,[k=1:K, sc=1:SC],-alpha_bid[sc,k]*alpha_bid_fix[k]-mu_down_k[k,sc]+mu_up_k[k,sc]+lambda[findall(x->x==1, psi_k[k,:])[1]]==0) 

@constraint(A2_22,[s=1:S, sc=1:SC],alpha_offer_s[s]-mu_down_s[s,sc]+mu_up_s[s,sc]-lambda[findall(x->x==1, psi_s[s,:])[1]]==0)

@constraint(A2_22,[o=1:O, sc=1:SC],alpha_offer_o[sc,o]*alpha_offer_o_fix[o]-mu_down_o[o,sc]+mu_up_o[o,sc]-lambda[findall(x->x==1, psi_o[o,:])[1]]==0)
...
@constraint(A2_22,[n=1:N],sum(d[k,sc] for sc=1:SC, k=1:K if psi_k[k,n]==1) +sum(Sys_power_base*B[n,m]*(theta[n]-theta[m]) for m=1:N if Omega[n,m]==1) -sum(p_s[s] for s=1:S if psi_s[s,n]==1)- sum(p_o[o,sc] for sc=1:SC, o=1:O if psi_o[o,n]==1)==0)
...
@constraint(A2_22,[sc=1:SC, n=1:N,m=1:N;Omega[n,m]==1],mu_up_nm[sc,n,m]<=(1-u_up_nm[sc,n,m])*M) 
...
if termination_status(A2_22) == MOI.OPTIMAL
    ...
    SW= sum(alpha_bid[k,sc]*alpha_bid_fix[k]*value.(d[k,sc]) for k=1:K, sc=1:SC) 
        - sum(alpha_offer_o[sc,o]*alpha_offer_o_fix[o]*value.(p_o[o,sc]) for o=1:O, sc=1:SC)
        - sum(value.(alpha_offer_s[s])*value.(p_s[s,sc]) for s=1:S, sc=1:SC)
