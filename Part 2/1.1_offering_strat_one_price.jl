...
@objective(A2_11, Max, sum(prob[s] *    (scenarios[s][2][t] * p_DA[t] 
                                        + (1-scenarios[s][3][t]) * coef_high * scenarios[s][2][t] * (delta_up[t,s] - delta_down[t,s])
                                        + scenarios[s][3][t] * coef_low * scenarios[s][2][t] * (delta_up[t,s] - delta_down[t,s])) for t=1:T, s in generated_values))
