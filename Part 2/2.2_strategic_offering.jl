...
@constraint(A2_22,[k=1:K],-alpha_bid[k]-mu_down_k[k]+mu_up_k[k]+lambda[findall(x->x==1, psi_k[k,:])[1]]==0)  #[1]

@constraint(A2_22,[s=1:S],alpha_offer_s[s]-mu_down_s[s]+mu_up_s[s]-lambda[findall(x->x==1, psi_s[s,:])[1]]==0) #[2]

@constraint(A2_22,[o=1:O],alpha_offer_o[o]-mu_down_o[o]+mu_up_o[o]-lambda[findall(x->x==1, psi_o[o,:])[1]]==0) #[3]

@constraint(A2_22,[n=1:N], sum(Sys_power_base*B[n,m]*(lambda[n]-lambda[m]) for m=1:N if Omega[n,m]==1) + sum(Sys_power_base*B[n,m]*(mu_up_nm[n,m]-mu_up_nm[m,n]) for m=1:N if Omega[n,m]==1) + (n == 1 ? gamma[1] : 0) ==0) #[4]
...
#2_KKT_conditions
@constraint(A2_22,[n=1],theta[n]==0)  #[6]

@constraint(A2_22,[n=1:N],sum(d[k] for k=1:K if psi_k[k,n]==1) +sum(Sys_power_base*B[n,m]*(theta[n]-theta[m]) for m=1:N if Omega[n,m]==1) -sum(p_s[s] for s=1:S if psi_s[s,n]==1)- sum(p_o[o] for o=1:O if psi_o[o,n]==1)==0) #[5]


#3_KKT_conditions_linearized
@constraint(A2_22,[k=1:K],d[k]<=u_down_k[k]*M[1])                                                                           #[7]
@constraint(A2_22,[s=1:S],p_s[s]<=u_down_s[s]*M[2])                                                                         #[8]
@constraint(A2_22,[o=1:O],p_o[o]<=u_down_o[o]*M[3])                                                                         #[9]

@constraint(A2_22,[k=1:K],mu_down_k[k]<=(1-u_down_k[k])*M[4])                                                               #[7]
@constraint(A2_22,[s=1:S],mu_down_s[s]<=(1-u_down_s[s])*M[5])                                                               #[8]
@constraint(A2_22,[o=1:O],mu_down_o[o]<=(1-u_down_o[o])*M[6])                                                               #[9]
...
@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],Sys_power_base*B[n,m]*(theta[n]-theta[m])+F[n,m]>=0)                          #[13]  
@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],Sys_power_base*B[n,m]*(theta[n]-theta[m])+F[n,m]<=u_down_nm[n,m]*M[7])        #[13] 
@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],mu_down_nm[n,m]<=(1-u_down_nm[n,m])*M[8])                                     #[13]


@constraint(A2_22,[k=1:K],D_max_k[k]-d[k]>=0)                                                                               #[10]
@constraint(A2_22,[s=1:S],P_max_s[s]-p_s[s]>=0)                                                                             #[11]
@constraint(A2_22,[o=1:O],P_max_o[o]-p_o[o]>=0)                                                                             #[12]

@constraint(A2_22,[k=1:K],D_max_k[k]-d[k]<=u_up_k[k]*M[9])                                                                  #[10]
@constraint(A2_22,[s=1:S],P_max_s[s]-p_s[s]<=u_up_s[s]*M[10])                                                               #[11]
@constraint(A2_22,[o=1:O],P_max_o[o]-p_o[o]<=u_up_o[o]*M[11])                                                               #[12]

@constraint(A2_22,[k=1:K],mu_up_k[k]<=(1-u_up_k[k])*M[12])                                                                  #[10]
@constraint(A2_22,[s=1:S],mu_up_s[s]<=(1-u_up_s[s])*M[13])                                                                  #[11]
@constraint(A2_22,[o=1:O],mu_up_o[o]<=(1-u_up_o[o])*M[14])                                                                  #[12]

@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],-Sys_power_base*B[n,m]*(theta[n]-theta[m])+F[n,m]>=0)                         #[14]
@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],-Sys_power_base*B[n,m]*(theta[n]-theta[m])+F[n,m]<=u_up_nm[n,m]*M[15])        #[14] 
@constraint(A2_22,[n=1:N,m=1:N;Omega[n,m]==1],mu_up_nm[n,m]<=(1-u_up_nm[n,m])*M[16])                                        #[14]
